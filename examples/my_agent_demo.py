...
async def main():
    print("=== Using InfoAssistantAgent with Gemini ===")
    ...
    # 设置Gemini为默认提供商
    manager.default_provider = "gemini"
    manager.fallback_chain = ["gemini", "openai"]  # Gemini优先
    
    print(f"✓ Default provider: {manager.default_provider}")
    print(f"✓ Fallback chain: {manager.fallback_chain}")

    info_agent = MyInfoAgent(
        llm=ChatBot(
            llm_provider="gemini",
            model_name="gemini-2.5-flash"  # 使用免费额度充足的模型
        )
    )
